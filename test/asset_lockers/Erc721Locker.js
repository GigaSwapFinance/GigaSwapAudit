...
describe("Erc721Locker", function () {
    let owner;
    let acc1;
    let acc2;
    let addrs;
    ...
    beforeEach(async () => {
        ...
        // локер
        locker = await (await ethers.getContractFactory("Erc721Locker")).deploy(feeSettings.address);
        // тестовый токен
        token = await (await ethers.getContractFactory("Erc721TestToken")).deploy();
    });
    ...
    it("эвент лока", async () => {
        ...
        const lockId = 1;
        await token.mint();
        await token.approve(locker.address, lockId);
        await expect(locker.lockSeconds(token.address, [lockId], lockSeconds))
    ...
    describe("делаем лок", function () {
        ...
        beforeEach(async () => {
            await token.mint();
            await token.approve(locker.address, lockId);
            await locker.lockSeconds(token.address, [lockId], lockSeconds)
        });
        ...
        it("ассет перешел на локер", async () => {
            ...
            expect(lockerBalance).to.equals(1);
        });
