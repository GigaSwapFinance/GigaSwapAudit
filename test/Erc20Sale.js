...
describe("Erc20Sale:", function () {
    ...
    describe("создание позиции", function () {
        ...
        it("инкремент id", async () => {
            ...
            await expect(erc20Sale.createPosition(token1.address, token2.address, priceNom, pricceDenom, token1Count, positionFlags, buyLimit, whiteList, packetClaimTime))
                .to.emit(erc20Sale, 'OnCreate')
    ...
    describe("создание позиции с переводом на карман", function () {
        var token1;
        var token2;
        var positionId;
        var position;
        ...
        it("получаем таймер кармана", async () => {
            expect(await erc20Sale.getPositionPacketClaimTime(positionId)).to.eq(packetClaimTime);
        });
        it("токены овнера переведены на контракт, после создания позиции", async () => {
            expect(await token1.balanceOf(erc20Sale.address)).to.eq(1000);
            expect(await token2.balanceOf(erc20Sale.address)).to.eq(0);
        ...
        it("закуп создает стак на кармане, который можно забрать по прошествии указанного времни, при создании позиции", async () => {
            const buyCount = 100;
            await token2.connect(acc2).mint(1000);
            await token2.connect(acc2).approve(erc20Sale.address, 1000);
        ...
        describe("покупка 100 штук", function () {
            ...
            it("тот, кто купил - получает токен в карман", async () => {
