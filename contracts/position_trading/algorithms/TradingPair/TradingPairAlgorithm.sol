    ...
    ITradingPairAlgorithm
{
    ...
    function createAlgorithm(
        ...
        if (out.estimateDecimals() > decimals)
            decimals = out.estimateDecimals();
    ...
    function _swap(
        ...
        // transfer the asset calculations
        uint256 devFee = (vars.buyCount *
            positionsController.getFeeSettings().feePercent()) /
            positionsController.getFeeSettings().feeDecimals();
        uint256 outputCountToTransferFithFee = vars.buyCount - devFee;

        // count the old price
        vars.newPrice =
            (input.count() * priceDecimals) /
            (output.count() - outputCountToTransferFithFee);
